...
- **Working copy for builds and edits:** use the **`firmware/`** folder at the repo root. Open `firmware/microcontroller-original-src/` in PlatformIO (or the `.ino` in Arduino IDE), make changes there, and build `firmware.bin` from that tree.
