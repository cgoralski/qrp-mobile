...
2. **Install DRA818:**
   ...
   - **Fix DRA818 compilation errors (ESP32 3.x):**  
     If Verify fails with errors in `DRA818.cpp` (e.g. default arguments, static linkage, or `DRA818_BAND_FLAG` / `rssi()` not declared), use **both** patched files so the header and implementation match:
     - **Option A (recommended):** Replace **both** files in your installed DRA818 library with the patched pair from this repo:
       - Copy **[DRA818-patched/DRA818.cpp](DRA818-patched/DRA818.cpp)** → `DRA818/src/DRA818.cpp`
       - Copy **[DRA818-patched/DRA818.h](DRA818-patched/DRA818.h)** → `DRA818/src/DRA818.h`  
       (e.g. on macOS the library path is often `~/Documents/Arduino/libraries/DRA818/src/` or `~/Documents/Arduino/sketches/libraries/DRA818/src/`.)
     - **Option B:** Apply the edits in **[DRA818-FIX.md](DRA818-FIX.md)** to your installed library; if your header is older and lacks `DRA818_BAND_FLAG` or `rssi()`, replace `DRA818.h` with the one from [DRA818 v1.0.1](https://github.com/fatpat/arduino-dra818/releases/tag/v1.0.1) as well.
...
## Troubleshooting: No RX audio in the browser

The board only sends RX audio (Opus) when it is in **MODE_RX**. It enters MODE_RX only after:

1. **The app sends COMMAND_HOST_GROUP (0x03)** with a valid frequency. The web app matches the Android handshake: it sends GROUP only **after** receiving COMMAND_VERSION from the board (so the board has left doConfig() and is ready). In the browser console you should see `[KV4P] version received, windowSize=...` then `[KV4P] Sending GROUP freqTx=... freqRx=... squelch=0`. If you see `[KV4P] GROUP not sent: invalid frequency`, the channel frequency is invalid.
2. **The firmware receives GROUP** and programs the radio with `sa818.group(...)`. The handler runs `while (!sa818.group(...));` and only then calls `setMode(MODE_RX)`. If the radio module (DRA818/SA818) does not respond (wrong wiring, not powered, wrong band), this loop **blocks indefinitely** and the board never enters MODE_RX — so no RX audio, and the board appears not to respond to GROUP. Ensure the radio module is connected, powered, and on the correct UART pins; if it never ACKs, the firmware will stay stuck here (watchdog may reset the board after ~10 s).
3. **`initI2SRx()` runs** when entering MODE_RX. On some setups (e.g. certain audio-tools versions) this can hang; if the board stops responding after GROUP, try a firmware build that does not set `config.adc_channels[0]` (see the comment in `rxAudio.h`).

Once in MODE_RX, the board sends Opus frames as COMMAND_RX_AUDIO (0x07). In the console you should then see `[KV4P] RX audio chunk #1`, `#2`, etc. If you **never** see those lines, the board is not sending 0x07: the firmware RX path is not producing audio (check that the board enters MODE_RX after GROUP, that `initI2SRx()` completes, and that the ADC input channel matches your hardware—e.g. GPIO 34 = channel 6; the backup firmware uses the library default channel which may be wrong). If you see RX audio chunks but hear no sound, the issue is playback (AudioContext, decoder, or device volume).
