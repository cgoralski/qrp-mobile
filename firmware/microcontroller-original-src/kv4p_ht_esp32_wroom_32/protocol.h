...
/** Current host transport (Serial or WiFi WebSocket). Set before parser.loop() so responses go to the right client. */
extern Stream* g_hostStream;

/**
 * Send a command with params
 * Format: [DELIMITER(8 bytes)] [CMD(1 byte)] [paramLen(2 bytes LE)] [param data(N bytes)]
 */
void __sendCmdToHost(SndCommand cmd, const uint8_t *params, size_t paramsLen) {
  if (!g_hostStream) return;
  if (paramsLen > PROTO_MTU) paramsLen = PROTO_MTU;
  g_hostStream->write(COMMAND_DELIMITER, DELIMITER_LENGTH);
  g_hostStream->write((uint8_t*) &cmd, 1);
  uint16_t len = paramsLen;
  g_hostStream->write((uint8_t*) &len, sizeof(len));
  if (paramsLen > 0) g_hostStream->write(params, paramsLen);
}
